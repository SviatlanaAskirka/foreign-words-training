const words = [{
    ...
    },
    {
        word: "school",
        translate: "школа",
        example: "She went to school by bus",
    },
    {
        word: "sun",
        translate: "солнце",
    ...
    },
    {
        word: "apple",
        translate: "яблоко",
        example: "Apple is my favourite fruit",
    },
    {
        word: "dress",
        translate: "платье",
        example: "I like my new dress",
    },
];
...
function getExamProgress(value) {
...
examCards.addEventListener("click", (event) => {
    ...
    if (click === false) {
        ...
        if (firstCardIndex === secondCardIndex) {
            document.querySelector('#correct-percent').textContent = getExamProgress(endIndex) + '%';
            document.querySelector('#exam-progress').value = getExamProgress(endIndex);
            endIndex++;
            ...
            if (endIndex === size) {
                clearInterval(timer);
                document.querySelector('.motivation').textContent = 'Супер! Ты — молодец!';
            }
            click = false;
